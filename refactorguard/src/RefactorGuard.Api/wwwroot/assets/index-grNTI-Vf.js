...
`;const w=g("repoPath"),$=g("useLlm"),o=h("output"),b=h("statusPill"),u=h("reports");l("statusButton",L);l("previewButton",P);l("reviewButton",E);l("analyzeButton",R);l("refreshReportsButton",p);p();async function L(){await c("Checking gpu-search...",async()=>{const e=await i("/api/search/status");o.innerHTML=f(e.isAvailable?"gpu-search is available":"gpu-search is unavailable",[`Health: ${e.health?.status??"n/a"}`,`Backend: ${e.stats?.backend??"n/a"}`,`Device: ${e.stats?.device??"n/a"}`,`Indexed files: ${e.stats?.indexedFileCount??"n/a"}`,e.error?`Error: ${e.error}`:""].filter(Boolean))})}async function P(){await c("Loading diff preview...",async()=>{const e=await i("/api/review/diff/preview",{method:"POST",body:JSON.stringify({repoPath:w.value})});o.innerHTML=`
  ...
  `}function I(e){return`${e.status} ${e.path} (+${e.additions}/-${e.deletions})`}function l(e,t){h(e).addEventListener("click",()=>{t()})}function g(e){const t=document.getElementById(e);if(!(t instanceof HTMLInputElement))throw new Error(`${e} input was not found.`);return t}function h(e){const t=document.getElementById(e);if(!t)throw new Error(`${e} element was not found.`);return t}function m(e){b.textContent=e}function s(e){return e.replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}
