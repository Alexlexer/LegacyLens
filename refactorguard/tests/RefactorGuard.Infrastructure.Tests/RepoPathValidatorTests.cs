...
public sealed class RepoPathValidatorTests : IDisposable
{
    ...
    {
        var outside = Directory.CreateDirectory(Path.Combine(Path.GetTempPath(), $"refactorguard-outside-{Guid.NewGuid():N}")).FullName;
        var validator = new RepoPathValidator([_root]);
    ...
    {
        return Directory.CreateDirectory(Path.Combine(Path.GetTempPath(), $"refactorguard-{Guid.NewGuid():N}")).FullName;
    }
}
